...
// 축별로 결이 다른 한줄 코멘트 풀 — 획일적이지 않게 구성
const AXIS_COMMENTS: Record<string, [number, string][]> = {
  관심도: [
    [75, "너한테 진짜 끌리고 있는 게 보인다냥."],
    [60, "관심은 살아있어. 식지는 않았어."],
    [45, "적당히 신경 쓰는 정도, 깊게는 아직."],
    [30, "관심이 빠지는 중일 수도 있다냥. 짚고 가야 해."],
    [0, "솔직히 너한테 시선이 거의 없어 보여."],
  ],
  적극성: [
    [75, "먼저 다가오는 사람이라냥. 망설임 없어."],
    [60, "먼저 연락도 해. 나쁘진 않아."],
    [45, "받아주긴 하는데 먼저 움직이진 않아."],
    [30, "거의 너가 시작해야 가는 그림이다냥."],
    [0, "먼저 다가오는 신호가 거의 없어."],
  ],
  반응성: [
    [75, "리액션 빠르고 풍부해. 듣고 있다는 게 느껴진다냥."],
    [60, "답장 톤은 살아있어. 무성의하진 않아."],
    [45, "답장은 오는데 톤이 평이해진 느낌."],
    [30, "단답·드라이로 흐르는 중이다냥. 주의."],
    [0, "거의 무반응. 신호가 너무 약해."],
  ],
  친밀감: [
    [75, "속마음도 꺼내는 단계. 거리감이 짧다냥."],
    [60, "겉도는 얘기 말고 진짜 얘기도 해주는 편."],
    [45, "딱 친구 정도. 깊은 얘기는 아직."],
    [30, "속얘기는 피하고 표면만 도는 분위기다냥."],
    [0, "벽이 두꺼워. 너한테 거리 두는 중이야."],
  ],
  일관성: [
    [75, "꾸준해. 변덕 없는 사람이다냥."],
    [60, "전반적으로 안정적. 큰 흔들림은 없어."],
    [45, "기복이 좀 있어. 매번 같진 않아."],
    [30, "들쭉날쭉해서 너 머리 아프게 하는 패턴이야."],
    [0, "어제랑 오늘이 너무 달라. 불안 포인트냐옹."],
  ],
  미래지향: [
    [75, "다음을 같이 그리는 중. 진심이다냥."],
    [60, "가까운 약속은 잘 잡아. 발은 걸친 셈."],
    [45, "당장 약속만 잡고 먼 그림은 안 보여."],
    [30, "미래 얘기는 살짝 피해. 거리 둔다냥."],
    [0, "장기 그림이 거의 안 보여. 발만 걸친 거야."],
  ],
};

function commentFor(label: string, v: number) {
  const list = AXIS_COMMENTS[label];
  if (!list) return `${label} ${v}점. 평균선 근처에 있어.`;
  for (const [threshold, msg] of list) {
    if (v >= threshold) return msg;
  }
